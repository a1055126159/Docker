...
if [[ $DB_CONNECTABLE -eq 0 ]]; then
    ...
    if [[ DB_EXISTS -eq 1 ]]; then
        ...
        if [[ RET -ne 0 ]]; then
            echo "Cannot create database for Hifone"
            exit RET
        fi
        if [ -f /initial_db.sql ]; then
            echo "=> Loading initial database data to $MYSQL_INSTANCE_NAME"
            RET=$(mysql -u$MYSQL_USERNAME -p$MYSQL_PASSWORD -h$MYSQL_DB_HOST -P$MYSQL_DB_PORT $MYSQL_INSTANCE_NAME < /db.sql)
            if [[ RET -ne 0 ]]; then
                echo "Cannot load initial database data for Hifone"
                exit RET
            fi
        fi
        echo "=> Done!"    
